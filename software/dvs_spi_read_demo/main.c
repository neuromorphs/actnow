...
/* Exercises spi_prog's read direction: stages an address via spi_prog's
   address register, then reads its data register -- that read triggers
   the actual SPI READ transaction (R/W=0 + staged address out, 32 bits
   back over miso). Compares the result against a known value and drives
   a self-checking pattern onto GPIO: 0b1111 if it matches, 0b0000 if it
   doesn't. */
